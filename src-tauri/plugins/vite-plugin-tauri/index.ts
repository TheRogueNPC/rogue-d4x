...
async function initTauri() {
	...
	await TauriCli.run(
		[
			...
			ta uriVersion === 1 ? "--dev-path" : "--dev-url",
			`Injected: by vite-plugin-tauri, you can change this if you want to use tauri cli directly`,
		],
		"vite-tauri",
	);
	...
	return [
		...
		},
		{
			...
			async closeBundle() {
				...
				if (!tauriConfPath) {
					await initTauri();
					ta uriConfPath = getTauriConfPath();
				}
				...
				args = [
					...
					JSON.stringify({
						build: {
							// at this point, `ta uriConfPath` can't be null
							[ta uriVersion === 1 ? "distDir" : "frontendDist"]: path.relative(
								dirname(ta uriConfPath!),
